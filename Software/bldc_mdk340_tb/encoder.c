...
#define AS5047_SAMPLE_RATE_HZ		20000  //20000

#if AS5047_USE_HW_SPI_PINS
	#ifdef HW_SPI_DEV//USE
...
#define USE_ENCODER_FLT_SEL 1

#define MED_WIDTH_NUM 50
#define MED_FIL_ITEM  50
#if USE_ENCODER_FLT_SEL
int MID_FLT_ENCODER=20;
#else
int MID_FLT_ENCODER=3;
#endif
float encoder_timer=0;
int med_filter_tmp[MED_FIL_ITEM][MED_WIDTH_NUM ];
int med_filter_out[MED_FIL_ITEM];
char med_fil_cnt[MED_FIL_ITEM];
float angle_flt;
// 1  2  3                                9
int Moving_Median(char item,char width_num,int in)
{
	char i,j;
	int t;
	...
	else
	{
		...
		return ( tmp[(char)width_num/2] );
	}
}

//((float)last_enc_angle * 360.0f) / 16384.0f);
float encoder_read_deg(void) {
	static float angle = 0.0f;

	switch (mode) {
	case ENCODER_MODE_ABI:
		angle = angle_flt= last_enc_angle=((float)HW_ENC_TIM->CNT * 360.0f) / (float)enc_counts;
		break;
	...
	case ENCODER_MODE_AS5047P_SPI:
		...
		angle = angle_flt;
	  #else
	  angle = angle_flt= (float)last_enc_angle* 0.02197;
	  #endif	  
		break;
...
float encoder_read_deg_flt(void) {
	switch (mode) {
	case ENCODER_MODE_ABI:
		angle_flt = last_enc_angle=((float)HW_ENC_TIM->CNT * 360.0f) / (float)enc_counts;
		break;

	case ENCODER_MODE_AS5047P_SPI:
		#if USE_ENCODER_FLT_SEL
	  angle_flt= (float)Moving_Median(0,MID_FLT_ENCODER,last_enc_angle )* 0.02197;
		#else
	  angle_flt= (float)last_enc_angle * 0.02197;
	  #endif
	  break;
...
void encoder_tim_isr(void) {
  ...
  static float angle_r;
	...
	if(pos!=0){
	  ...
	  last_enc_angle = pos;
		#else
		last_enc_angle = Moving_Median(0,MID_FLT_ENCODER,pos);
		#endif
 } else
	err=1;
}
