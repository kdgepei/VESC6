...
volatile float speed_pid_set_rpm;
volatile float pos_pid_set_pos;
...
static THD_FUNCTION(rpm_thread, arg) {
	...
	for (;;) {
		...
		if (rpm_dep.comms != 0) {
			...
			rpm_now = (comms * MCPWM_RPM_TIMER_FREQ * 60.0f) / (time_at_comm * 6.0f);
		} else {
			// In case we have slowed down
			float rpm_tmp = (MCPWM_RPM_TIMER_FREQ * 60.0f) / ((float) TIM2 ->CNT * 6.0f);
...
void mcpwm_adc_int_handler(void *p, uint32_t flags) {
	...
	if (encoder_is_configured()) {
		run_pid_control_pos(1.0f / switching_frequency_now);
	}

	last_adc_isr_duration = (float)TIM12->CNT / 10000000.0f;
}
