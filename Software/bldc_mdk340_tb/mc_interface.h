...
typedef struct {
	float iq,id,i_all,i_bus;
	float iq_set,id_set;
	float duty,vbus;
	int round;
	float pos,posr,pos_last,pos_all[2],spd;
}motor_oldx;

typedef struct {
	char board_id,round_div,en_rx_can,cal_now;
	float current_kp,current_ki;
  float spd_kp,spd_ki,spd_kd;
	float pos_kp,pos_ki,pos_kd;
	float set_pos,set_spd,set_i;
	float pos_dt;
	float flt_spd_est,flt_pos_err,flt_current;
}param_oldx;

typedef struct {
	char m_state,m_statef,m_state_param,connect,run_heart,err;
	float loss_cnt,err_cnt;
	motor_oldx motor;
	param_oldx param;
	float set_pos,set_pos_flt,set_spd,set_i;
	
	float max_spd,max_i;
  char last_run_heart;
} bldc_oldx;
extern bldc_oldx bldc;
