...
#define SERIAL_RX_BUFFER_SIZE		64//1024
...
static void process_packet(unsigned char *data, unsigned int len) {
	...
	//commands_process_packet(data, len);//<<--------------------·¢ËÍÐ­Òé
}
...
void send_packet_back(unsigned char *data, unsigned int len) {
...
void send_packet(unsigned char *data, unsigned int len) {
	// Wait for the previous transmission to finish.
	if (HW_UART_DEV.txstate == UART_TX_ACTIVE) {
...
//			for(cnt_pack=0;cnt_pack<SERIAL_RX_BUFFER_SIZE;cnt_pack++)
//			  USART6_Packed(serial_rx_buffer[cnt_pack]);
