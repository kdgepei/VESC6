...
static THD_FUNCTION(output_thread, arg) {
	...
	for(;;) {
		...
		const float max_current_diff = mcconf->l_current_max * 0.2f;
		...
		// LEDs
		float x_axis = ((float)chuck_d.js_x - 128.0f) / 128.0f;
		if (out_val < -0.001f) {
			if (x_axis < -0.4f) {
		...
		if (ramp_time > 0.01f) {
			const float ramp_step = ((float)OUTPUT_ITERATION_TIME_MS * current_range) / (ramp_time * 1000.0f);
			...
			if (is_reverse) {
				if (fabsf(current_goal + current_highest_abs) > max_current_diff) {
					utils_step_towards(&goal_tmp2, -current_highest_abs, 2.0f * ramp_step);
				}
			} else {
				if (fabsf(current_goal - current_highest_abs) > max_current_diff) {
					utils_step_towards(&goal_tmp2, current_highest_abs, 2.0f * ramp_step);
			...
			if ((!is_decreasing || is_decreasing2) && fabsf(out_val) > 0.001f) {
