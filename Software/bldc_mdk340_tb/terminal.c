...
void terminal_process_string(char *str) {
	...
	if (strcmp(argv[0], "ping") == 0) {
		...
		commands_printf("Latest ADC duration: %.4f ms", (double)(mcpwm_get_last_adc_isr_duration() * 1000.0f));
		commands_printf("Latest injected ADC duration: %.4f ms", (double)(mc_interface_get_last_inj_adc_isr_duration() * 1000.0f));
		commands_printf("Latest sample ADC duration: %.4f ms\n", (double)(mc_interface_get_last_sample_adc_isr_duration() * 1000.0f));
		...
		for (int i = 0;i < CAN_STATUS_MSGS_TO_STORE;i++) {
			...
			if (msg->id >= 0 && UTILS_AGE_S(msg->rx_time) < 1.0f) {
				commands_printf("ID                 : %i", msg->id);
				commands_printf("RX Time            : %i", msg->rx_time);
				commands_printf("Age (milliseconds) : %.2f", (double)(UTILS_AGE_S(msg->rx_time) * 1000.0f));
		...
		if (argc == 5) {
			...
			if (current > 0.0f && current <= mcconf.l_current_max && min_erpm > 0.0f && duty > 0.02f && res >= 0.0f) {
				float linkage;
		...
		if (argc == 2) {
			...
			if (duty > 0.0f) {
				mcconf.motor_type = MOTOR_TYPE_FOC;
				mc_interface_set_configuration(&mcconf);
				const float res = (3.0f / 2.0f) * mcconf.foc_motor_r;
				...
				float linkage = (vq_avg - res * iq_avg) / (rpm_avg * ((2.0f * M_PI) / 60.0f));
