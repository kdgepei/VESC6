...
#if defined ARM_MDK || defined ARM_IAR
#if defined ARM_SINCOS
...
// Private variables
volatile mc_state m_state;// MC_STATE_RUNNING
...
static void run_pid_control_pos_spd(float angle_now, float angle_set, float dt);
static void run_pid_control_speed(float dt);
static void run_pid_control_speed_mine(float dt) ;
...
void mcpwm_foc_init(volatile mc_configuration *configuration) {
	...
	if(m_conf->foc_f_sw>30000)
		m_conf->foc_f_sw=30000;
...
void mcpwm_foc_encoder_detect(float current, bool print, float *offset, float *ratio, bool *inverted) {
	...
	for (int i = 0; i < it_rat; i++) {
		...
		if (print) {
			commands_printf("%.2f", (double)(diff * 180.0f / M_PI));
		}
...
float mcpwm_foc_measure_inductance(float duty, int samples, float *curr) {
	...
	return ((avg_voltage * t) / avg_current) * 1e6f * (2.0f /  3.0f);
}
...
bool mcpwm_foc_measure_res_ind(float *res, float *ind) {
	...
	m_conf->foc_f_sw = 10000.0f;
	m_conf->foc_current_kp = 0.01f;
	m_conf->foc_current_ki = 10.0f;
	...
	float res_tmp = 0.0f;
	float i_last = 0.0f;
	for (float i = 2.0f;i < (m_conf->l_current_max / 2.0f);i *= 1.5f) {
	...
	if (i_last < 0.01f) {
		i_last = (m_conf->l_current_max / 2.0f);
	}

	*res = mcpwm_foc_measure_resistance(i_last, 200);

	m_conf->foc_f_sw = 3000.0f;
...
bool mcpwm_foc_hall_detect(float current, uint8_t *hall_table) {
	...
	for (int i = 0;i < 3;i++) {
		for (int i = 0;i < 360;i++) {
			m_phase_now_override = (float)i * M_PI / 180.0f;
	...
	for(int i = 0;i < 8;i++) {
		if (hall_iterations[i] > 30) {
			float ang = atan2f(sin_hall[i], cos_hall[i]) * 180.0f / M_PI;
...
void mcpwm_foc_adc_int_handler(void *p, uint32_t flags) {
	...
	float ic = ADC_curr_norm_value[2] * FAC_CURRENT;//-(ia + ib);
...
#else
	const float dt = 1.0f / (m_conf->foc_f_sw / 2.0f);
#endif
	...
	if (m_state == MC_STATE_RUNNING) {//<-------------------电机控制  电流环
		...
		m_motor_state.i_alpha = (2.0f / 3.0f) * ia - (1.0f / 3.0f) * ib - (1.0f / 3.0f) * ic;
		...
		case FOC_SENSOR_MODE_SENSORLESS:
			...
			if (!m_phase_override) {
				if (duty_abs < m_conf->foc_sl_d_current_duty) {
					id_set_tmp = utils_map(duty_abs, 0.0f, m_conf->foc_sl_d_current_duty,
							fabsf(m_motor_state.iq_target) * m_conf->foc_sl_d_current_factor, 0.0f);
				} else {
					id_set_tmp = 0.0f;
				}
			}
		...
		control_current(&m_motor_state, dt);//电流控制
	} else {
		// Track back emf
#ifdef HW_HAS_3_SHUNTS
	...
	if (encoder_is_configured()) {
		...
		angle_now = m_motor_state.phase * (180.0f / M_PI);
	}
...
//	}
	...
	if (m_state == MC_STATE_RUNNING) {
		//run_pid_control_pos_spd(m_pos_pid_now, m_pos_pid_set, dt);
...
static THD_FUNCTION(timer_thread, arg) {
	...
	for(;;) {
		...
		run_pid_control_speed_mine(dt);
		chThdSleepMilliseconds(dt_foc*1000);
	}
}
...
static void pll_run(float phase, float dt, volatile float *phase_var,
		volatile float *speed_var) {
	float rc = 1.0f/(2*PI*bldc.param.flt_spd_est);
...
float dead_current=0.2;//A
float dead_pos=0.2;//Degree
float FLT_SPD=5;
...
float current_stop_test[3]={0,0,0.5};//en , flag ,dead
static void control_current(volatile motor_state_t *state_m, float dt) {
	float c,s;
	//参数复制
	if(bldc.connect){
	if(bldc.param.current_kp!=0)
		m_conf->foc_current_kp=bldc.param.current_kp;
	if(bldc.param.current_ki!=0)
		m_conf->foc_current_ki=bldc.param.current_ki;
  if(bldc.param.pos_kp!=0)
		m_conf->p_pid_kp=bldc.param.pos_kp;
	if(bldc.param.pos_kd!=0)
		m_conf->p_pid_kd=bldc.param.pos_kd;
  if(bldc.param.spd_kp!=0)
		m_conf->s_pid_kp=bldc.param.spd_kp;
	if(bldc.param.spd_kd!=0)
		m_conf->s_pid_kd=bldc.param.spd_kd;
	}
  ...
  float rc = 1.0f/(2*PI*bldc.param.flt_current+0.00000001);
	float alpha = LIMIT(dt/(dt+rc), 0.0f, 1.0f);
	if(bldc.param.flt_current==0)
		alpha=1;
	state_m->id = (c * state_m->i_alpha + s * state_m->i_beta) *alpha    +(1-alpha)*state_m->id;
	state_m->iq = (c * state_m->i_beta  - s * state_m->i_alpha)*alpha    +(1-alpha)*state_m->iq;
	
	state_m->id_filter=state_m->id;state_m->iq_filter=state_m->iq;
	//UTILS_LP_FAST(state_m->id_filter, state_m->id, m_conf->foc_current_filter_const);UTILS_LP_FAST(state_m->iq_filter, state_m->iq, m_conf->foc_current_filter_const);

	float Ierr_d =my_deathzoom( state_m->id_target - state_m->id,dead_current);
	float Ierr_q =my_deathzoom( state_m->iq_target - state_m->iq,dead_current);
...
static void run_pid_control_pos(float angle_now, float angle_set, float dt) {
  ...
  static float cnt_t;
	static int flag=1;
	float rc = 1.0f/(2*PI*bldc.param.flt_pos_err);
	float alpha = LIMIT(dt/(dt+rc), 0.0f, 1.0f);
	float angle_now_use;
	// PID is off. Return.
	...
	cnt_t+=dt;
	if(test_pos[4]==1){
		test_pos[0]+=dt*test_pos[1];
	  if(test_pos[0]>360)test_pos[0]=0;
		set=test_pos[0];
	}
	else if(test_pos[4]==2){
		if(cnt_t>test_pos[2]){
			cnt_t=0;
			if(flag){flag=0;
				test_pos[0]=test_pos[3];}	
			else{flag=1;			
				test_pos[0]=-test_pos[3];}		
	  }
		set=test_pos[0];
	...
	// Compute parameters
	float error=utils_angle_difference(set, angle_now);
  ...
  error=LIMIT(my_deathzoom(error,dead_pos),-90,90);
	
	static float err;
	if(bldc.param.flt_pos_err==0)
		err=error;	
	else
		err=alpha*error+(1-alpha)*err;
	
	err=LIMIT(err,-90,90);
	p_term  = err *  m_conf->p_pid_kp;
	i_term += err * (m_conf->p_pid_ki * dt);
	...
	if (err == prev_error) {
		...
		d_term = (err - prev_error) * (m_conf->p_pid_kd/100. / dt_int);
		dt_int = 0.0f;
	}
...
static void run_pid_control_pos_spd(float angle_now, float angle_set, float dt) {
  ...
  error=my_deathzoom(error,dead_pos);
	static float err;
	err=alpha*error+(1-alpha)*err;
	bldc.set_spd=LIMIT(err*m_conf->p_pid_kp,-bldc.max_spd,bldc.max_spd);
}
...
static void run_pid_control_speed(float dt) {
	...
	d_term = (error - prev_error) * (m_conf->s_pid_kd/100. / dt) * (1.0f / 20.0f);
...
float spd_control_view[5];
float m_pos_pid_set_test=0;
float pid_spd_pos[4]={50, 0, 0, 1};
float FLT_SPD_EXP=5;
static void run_pid_control_speed_mine(float dt) {
	static float i_term = 0.0f, i_term_pos=0;;
	static float prev_error = 0.0f;
	static float angle_before;
	static float pid_pos_out;
	static char cnt_pos=0;
	float rc = 1.0f/(2*PI*FLT_SPD);
	float alpha = LIMIT(dt/(dt+rc), 0.0f, 1.0f);
	float rc_exp_spd = 1.0f/(2*PI*FLT_SPD_EXP);
	float alpha_exp_spd = LIMIT(dt/(dt+rc_exp_spd), 0.0f, 1.0f);
	float p_term;
	float d_term;
	float set;
	static float set_spd_flt;
  static float cnt_t;
	static int flag=1;
	// PID is off. Return.
	if (m_control_mode != CONTROL_MODE_SPEED) {
		pid_pos_out= i_term_pos= i_term = 0.0f;
		prev_error = 0.0f;
		return;
	}
	...
	set=m_pos_pid_set_test;
	
	//position control loop
	float error_pos = utils_angle_difference(set, bldc.motor.pos_all[1]);
  error_pos=LIMIT(my_deathzoom(error_pos,dead_pos),-90,90);
	
	float pos_p=error_pos*pid_spd_pos[0];
	float pos_d;
	i_term_pos+=error_pos*pid_spd_pos[1];
  if(cnt_pos++>1){cnt_pos=0;
	pid_pos_out=LIMIT(pos_p+i_term_pos+pos_d,-bldc.max_spd*0.866,bldc.max_spd*0.866);
	}
	//speed control loop
	float error_spd = utils_angle_difference(angle_now, angle_before)/dt;
	angle_before=angle_now;
	static float rpm ;
	rpm= error_spd*alpha+(1-alpha)*rpm;
	spd_control_view[0]=rpm;
	spd_control_view[1]=m_speed_pid_set_rpm;
	
	float set_spd,error;
	if(pid_spd_pos[3])//速度位置环控制
		set_spd=pid_pos_out;
	else
		set_spd=m_speed_pid_set_rpm;
	
	set_spd_flt= set_spd*alpha_exp_spd+(1-alpha_exp_spd)*set_spd_flt;
	spd_control_view[2]=set_spd_flt;
  error=set_spd_flt-rpm;
	...
	utils_truncate_number_abs(&p_term, 1.0f);
	utils_truncate_number_abs(&i_term, 1.0f - fabsf(p_term));
	//utils_truncate_number(&i_term, -0.5f, 0.5f);
...
static float correct_hall(float angle, float speed, float dt) {
	...
	// Hysteresis 5 % of total speed
	float hyst = m_conf->foc_sl_erpm * 0.1f;
	if (using_hall) {
		if (rpm_abs > (m_conf->foc_sl_erpm + hyst)) {
			using_hall = false;
		}
	...
	if (using_hall) {
		...
		if (ang_hall_int < 201) {
			static float ang_hall = 0.0f;
			float ang_hall_now = (((float)ang_hall_int / 200.0f) * 360.0f) * M_PI / 180.0f;
			...
			if (ang_hall_int_prev < 0) {
				...
				if (ang_hall_int_prev == -2) {
					...
					ang_hall = ((ang_hall_int / 200.0f) * 360.0f) * M_PI / 180.0f;
				}
				...
				ang_avg %= 200;
				ang_hall = (((float)ang_avg / 200.0f) * 360.0f) * M_PI / 180.0f;
			}
			...
			if (rpm_abs < 100) {
				...
				if (fabsf(diff) < ((2.0f * M_PI) / 12.0f)) {
					// Do interpolation
					ang_hall += speed * dt;
				} else {
					// We are too far away with the interpolation
					ang_hall -= diff / 100.0f;
				}
			}
