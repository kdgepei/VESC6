...
bldc_oldx bldc;
void my_init(void){//150T
bldc.param.current_kp=m_conf.foc_current_kp=0.68;//2;
bldc.param.current_ki=m_conf.foc_current_ki=66;//100;
	
bldc.param.pos_kp=m_conf.p_pid_kp=0.08;//0.08;
bldc.param.pos_ki=m_conf.p_pid_ki=0.001;	
bldc.param.pos_kd=m_conf.p_pid_kd=0.01;//0.0008;

bldc.param.spd_kp=m_conf.s_pid_kp=0.01;
bldc.param.spd_ki=m_conf.s_pid_ki=0.001;	
bldc.param.spd_kd=m_conf.s_pid_kd=0.0015;
	
m_conf.l_current_max=18;
m_conf.l_current_min=-18;
	
m_conf.foc_pll_kp=3500;//速度估计
m_conf.foc_pll_ki=40000;
	
//m_conf.foc_encoder_inverted=0;
//m_conf.foc_encoder_offset=330.8;
//m_conf.foc_encoder_ratio=11;//重要影响闭环 使用上位机校准
	
m_conf.l_min_vin=3.3*3;	
m_conf.s_pid_min_erpm=0;	
	
//我的参数	
bldc.param.board_id=0x12;

bldc.param.round_div=m_conf.p_pid_ang_div;//减速比
bldc.max_spd=12500;//直驱
bldc.max_i=m_conf.l_current_max;

bldc.param.pos_dt=0.02;
bldc.param.flt_pos_err=150;//100;
bldc.param.flt_current=150;//50;//15;

bldc.param.en_rx_can=1;
//bldc.m_statef=1;
//bldc.set_pos=-160;
...
void mc_interface_init(mc_configuration *configuration) {
	...
	// Initialize encoder
#if !WS2811_ENABLE
  //m_conf.m_sensor_port_mode=SENSOR_PORT_MODE_ABI;m_conf.m_encoder_counts=4000;
...
void mc_interface_mc_timer_isr(void) {
	...
	if (fabsf(current) > 1.0f) {
		...
		static float curr_diff_sum = 0.0f;
		static float curr_diff_samples = 0;

		curr_diff_sum += current_in / f_samp;
		curr_diff_samples += 1.0f / f_samp;

		if (curr_diff_samples >= 0.01f) {
			if (curr_diff_sum > 0.0f) {
...
//				m_phase_samples[m_sample_now] = (uint8_t)(ang / 360.0f * 250.0f);
			...
			m_curr_fir_samples[m_sample_now] = (int16_t)(mc_interface_get_tot_current() * (8.0f / FAC_CURRENT));
			m_f_sw_samples[m_sample_now] = (int16_t)(f_samp / 10.0f);
 ...
 */
#define NO_MOTOR_TEMP_SENSOR 1		//  没有电机温度传感器   
#define NO_FET_TEMP_SENSOR 0		//  没有管子温度传感器  
static void update_override_limits(volatile mc_configuration *conf) {
	const float v_in = GET_INPUT_VOLTAGE();
	const float rpm_now = mc_interface_get_rpm();

	UTILS_LP_FAST(m_temp_fet, NTC_TEMP(ADC_IND_TEMP_MOS), 0.1f);
	UTILS_LP_FAST(m_temp_motor, NTC_TEMP_MOTOR(conf->m_ntc_motor_beta), 0.1f);
	#if NO_FET_TEMP_SENSOR
	 m_temp_fet=66;
	#endif
  #if NO_MOTOR_TEMP_SENSOR
	 m_temp_motor=66;
	#endif
	// Temperature MOSFET
	...
	if (m_temp_fet < conf->l_temp_fet_start) {
		...
		maxc = utils_map(m_temp_fet, conf->l_temp_fet_start, conf->l_temp_fet_end, maxc, 0.0f);
	...
	if (m_temp_motor < conf->l_temp_motor_start) {
		...
		maxc = utils_map(m_temp_motor, conf->l_temp_motor_start, conf->l_temp_motor_end, maxc, 0.0f);
	...
	if (m_temp_fet < temp_fet_accel_start) {
		...
		lo_fet_temp_accel = utils_map(m_temp_fet, temp_fet_accel_start,
				temp_fet_accel_end, conf->l_current_max, 0.0f);
	}
	...
	if (m_temp_motor < temp_motor_accel_start) {
		...
		lo_motor_temp_accel = utils_map(m_temp_motor, temp_motor_accel_start,
				temp_motor_accel_end, conf->l_current_max, 0.0f);
	}
	...
	if (rpm_now < rpm_pos_cut_start) {
		...
		lo_max_rpm = utils_map(rpm_now, rpm_pos_cut_start, rpm_pos_cut_end, conf->l_current_max, 0.0f);
	}
	...
	if (rpm_now > rpm_neg_cut_start) {
		...
		lo_min_rpm = utils_map(rpm_now, rpm_neg_cut_start, rpm_neg_cut_end, conf->l_current_max, 0.0f);
	}
	...
	if (v_in > conf->l_battery_cut_start) {
		...
		lo_in_max_batt = utils_map(v_in, conf->l_battery_cut_start,
				conf->l_battery_cut_end, conf->l_in_current_max, 0.0f);
	}
...
static THD_FUNCTION(sample_send_thread, arg) {
	...
	for(;;) {
		...
		for (int i = 0;i < len;i++) {
			...
			buffer_append_float32_auto(buffer, ((float)m_ph1_samples[ind_samp] / 4096.0f * V_REG) * ((VIN_R1 + VIN_R2) / VIN_R2), &index);
			buffer_append_float32_auto(buffer, ((float)m_ph2_samples[ind_samp] / 4096.0f * V_REG) * ((VIN_R1 + VIN_R2) / VIN_R2), &index);
			buffer_append_float32_auto(buffer, ((float)m_ph3_samples[ind_samp] / 4096.0f * V_REG) * ((VIN_R1 + VIN_R2) / VIN_R2), &index);
			buffer_append_float32_auto(buffer, ((float)m_vzero_samples[ind_samp] / 4096.0f * V_REG) * ((VIN_R1 + VIN_R2) / VIN_R2), &index);
