...
void commands_process_packet(unsigned char *data, unsigned int len) {
...
#if SERVO_OUT_ENABLE
		ind = 0;
		servo_simple_set_output(buffer_get_float16(data, 1000.0f, &ind));
#endif
		break;
	...
	case COMM_DETECT_MOTOR_PARAM:
		ind = 0;
		detect_current = buffer_get_float32(data, 1e3f, &ind);
		detect_min_rpm = buffer_get_float32(data, 1e3f, &ind);
		detect_low_duty = buffer_get_float32(data, 1e3f, &ind);
	...
	case COMM_GET_DECODED_CHUK:
		...
		buffer_append_int32(send_buffer, (int32_t)(app_nunchuk_get_decoded_chuk() * 1000000.0f), &ind);
		commands_send_packet(send_buffer, ind);
		break;
